...
export class JobsService {
  ...
  async create(createJobDto: CreateJobDto, user: User): Promise<Job> {
    console.log('Creating job with user:', { userId: user.id, companyId: user.companyId });
    
    if (!user.id) {
      throw new Error('User ID is required');
    }

    const expirationDateValue = createJobDto.expirationDate ? `'${createJobDto.expirationDate}'` : 'NULL';
    const departmentIdValue = createJobDto.departmentId ? `'${createJobDto.departmentId}'` : 'NULL';
    
    const result = await this.jobsRepository.query(`
      INSERT INTO jobs (id, title, description, requirements, expiration_date, status, company_id, department_id, created_by, created_at, updated_at)
      VALUES (gen_random_uuid(), $1, $2, $3, ${expirationDateValue}, $4, $5, ${departmentIdValue}, $6, NOW(), NOW())
    ...
    await this.createLog(jobId, user.id, 'Vaga criada', 'status', undefined, JobStatus.DRAFT);

    return this.findOne(jobId, user.companyId);
  }

  async createWithAi(createJobWithAiDto: CreateJobWithAiDto, user: User): Promise<Job> {
    console.log('Creating job with AI:', { userId: user.id, companyId: user.companyId });
    ...
    // Chamar AI Service
    const aiResponse = await this.aiServiceClient.createJobFromPrompt(aiRequest);
    ...
    const createJobDto: CreateJobDto = {
      ...
      stages: aiResponse.stages?.map((s, index) => ({
        name: s.name,
  ...
  async findOne(id: string, userCompanyId: string): Promise<Job> {
    const job = await this.jobsRepository.findOne({
      where: { id },
      relations: ['company', 'department', 'createdBy', 'questions', 'stages', 'logs', 'logs.user'],
    });
    ...
    if (job.companyId !== userCompanyId) {
      throw new ForbiddenException('Acesso negado: vaga não pertence à sua empresa');
    }
  ...
  async update(id: string, updateJobDto: UpdateJobDto, user: User): Promise<Job> {
    ...
    const oldValues = { ...job };
    const logsToCreate: Array<{description: string, fieldName?: string, oldValue?: string, newValue?: string}> = [];
    ...
    if (updateJobDto.description && updateJobDto.description !== job.description) {
    ...
    if (updateJobDto.requirements && updateJobDto.requirements !== job.requirements) {
    ...
    if (updateJobDto.departmentId !== undefined && updateJobDto.departmentId !== job.departmentId) {
    ...
    return await this.jobsRepository.manager.transaction(async (entityManager) => {
      ...
      for (const logData of logsToCreate) {
        await entityManager.query(
          ...
          [job.id, user.id, logData.description, logData.fieldName || null, logData.oldValue || null, logData.newValue || null]
        );
      }
  ...
  async updateQuestions(jobId: string, questions: any[], userId: string): Promise<void> {
    ...
    await this.createLog(jobId, userId, 'Perguntas do processo seletivo atualizadas', 'questions');
  }

  async updateStages(jobId: string, stages: any[], userId: string): Promise<void> {
    ...
    await this.createLog(jobId, userId, 'Etapas do processo seletivo atualizadas', 'stages');
  }
  ...
  async createLog(jobId: string, userId: string, description: string, fieldName?: string, oldValue?: string, newValue?: string): Promise<void> {
    await this.jobLogsRepository.query(
      `INSERT INTO job_logs (job_id, user_id, description, field_name, old_value, new_value, created_at) 
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
      [jobId, userId, description, fieldName || null, oldValue || null, newValue || null]
    );
  }
  ...
  async publish(id: string, user: User): Promise<Job> {
    ...
    await this.createLog(job.id, user.id, 'Vaga publicada', 'status', JobStatus.DRAFT, JobStatus.PUBLISHED);
  ...
  async close(id: string, user: User): Promise<Job> {
    ...
    if (job.status !== JobStatus.PUBLISHED) {
      throw new BadRequestException('Apenas vagas publicadas podem ser fechadas');
    }
    ...
    await this.jobsRepository.save(job);
    await this.createLog(job.id, user.id, 'Vaga fechada', 'status', JobStatus.PUBLISHED, JobStatus.CLOSED);
